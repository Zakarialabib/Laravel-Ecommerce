<div>
    ...
    <x-table>
        <x-slot name="thead">
            ...
            <x-table.th sortable wire:click="sortBy('link')" :direction="$sorts['link'] ?? null">
                {{ __('Link') }}
        ...
        <x-table.tbody>
            @forelse($sociallinks as $id=>$sociallink)
                <x-table.tr>
                    <x-table.td>
                        {{ $id }}
                    </x-table.td>
                    ...
                    <x-table.td>
                        <livewire:toggle-button :model="$sociallink" field="status" key="{{ $sociallink->id }}" />
                    </x-table.td>
                    <x-table.td>
                        <x-dropdown
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-32 p-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link data-href="{{ route('admin-sociallink-edit', $sociallink->id) }}"> 
                                    <i class="fas fa-edit"></i>
                                    {{ __('Edit') }}
                                </x-dropdown-link>
                                <x-dropdown-link href="javascript:;"
                                    data-href="{{ route('admin-sociallink-delete', $sociallink->id) }}" data-toggle="modal"
