        ...
        <x-table.tbody>
            @forelse($partners as $partner)
                <x-table.tr>
                    ...
                    <x-table.td>
                        {{ $partner->language->name }}  <img src="{{ flagImageUrl($partner->language->code) }}">
                    </x-table.td>
                    <x-table.td>
                        <livewire:toggle-button :model="$partner" field="status" key="{{ $partner->id }}" />
                    </x-table.td>
                    <x-table.td>
                        <div class="inline-flex">
                            ...
                            <button
                                ...
                                type="button" wire:click="confirm('delete', {{ $partner->id }})"
                                wire:loading.attr="disabled">
                                {{__('Delete')}}
                            </button>
                            <button
                                class="font-bold  bg-orange-500 border-orange-800 hover:bg-orange-600 active:bg-orange-700 focus:ring-orange-300 uppercase justify-center text-xs py-2 px-3 rounded shadow hover:shadow-md mr-1 ease-linear transition-all duration-150 cursor-pointer text-white"
                                type="button" wire:click="confirm('clone', {{ $partner->id }})"
                                wire:loading.attr="disabled">
                                {{__('Clone')}}
                            </button>
