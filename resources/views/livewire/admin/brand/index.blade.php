        ...
        <x-table.tbody>
            @forelse($brands as $id=>$brand)
                <x-table.tr>
                    ...
                    <x-table.td>
                        @php($photo = $brand->photo ? url('assets/images/partner/'.$brand->photo):url('assets/images/noimage.png'))
                    ...
                    <x-table.td>
                        <x-dropdown
                            ...
                            <x-slot name="content">
                                <x-dropdown-link data-href="{{ route('admin-brand-edit', $brand->id) }}"
                                    class="edit" data-toggle="modal" data-target="#modal1"> <i
                                        class="fas fa-edit"></i>
                                    {{ __('Edit') }}
                                </x-dropdown-link>
                                <x-dropdown-link href="javascript:;"
                                    data-href="{{ route('admin-brand-delete', $brand->id) }}" data-toggle="modal"
