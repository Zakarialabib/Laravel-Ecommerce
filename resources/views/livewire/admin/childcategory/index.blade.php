<div>
    ...
    <x-table>
        ...
        <x-table.tbody>
            @forelse($childcategories as $id=>$childcategory)
                <x-table.tr>
                    <x-table.td>
                        {{ $id }}
                    </x-table.td>
                    ...
                    <x-table.td>
                        <div class="action-list">
                            <a href="javascript;;"
                                data-href="'{{ route('admin-attr-createForChildcategory', $childcategory->id) }}"
                                class="attribute text-white" data-toggle="modal" data-target="#attribute"> <i
                                    class="fas fa-edit"></i>{{ __('Create') }}</a>
                            @if ($childcategory->attributes()->count() > 0)
                                <a href="{{ route('admin-attr-manage', $childcategory->id), '?type=childcategory' }}"
                                    class="edit">
                    ...
                    <x-table.td>
                            <livewire:toggle-button :model="$childcategory" field="status" key="{{ $childcategory->id }}" />                        
                    </x-table.td>
                    <x-table.td>
                        <x-dropdown
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-32 p-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link data-href="{{ route('admin-childcat-edit', $childcategory->id) }}" class="edit"
                                    data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit"></i>
                                    {{ __('Edit') }}
                                </x-dropdown-link>
                                <x-dropdown-link href="javascript:;"
                                    data-href="{{ route('admin-childcat-delete', $childcategory->id) }}" data-toggle="modal"
