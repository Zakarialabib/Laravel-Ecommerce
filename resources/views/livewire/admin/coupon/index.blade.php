        ...
        <x-table.tbody>
            @forelse($coupons as $id=>$coupon)
                <x-table.tr>
                    ...
                    <x-table.td>
                        @php($type = $data->type == 0 ? 'Discount By Percentage' : 'Discount By Amount')
                        {{ $type }}
                    </x-table.td>
                    <x-table.td>
                        @php($price = $data->type == 0 ? $data->price . '%' : \PriceHelper::showAdminCurrencyPrice($data->price * $this->curr->value))
                        {{ $price }}
                    </x-table.td>
                    <x-table.td>
                        {{ $coupon->used }}
                    ...
                    <x-table.td>
                        <livewire:toggle-button :model="$coupon" field="status" key="{{ $coupon->id }}" />
                    </x-table.td>
                    <x-table.td>
                        <x-dropdown
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-32 p-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link href="{{ route('admin-coupon-edit', $coupon->id) }}"> <i
                                        class="fas fa-edit"></i>
                                    {{ __('Edit') }}
                                </x-dropdown-link>
                                <x-dropdown-link href="javascript:;"
                                    data-href="{{ route('admin-coupon-delete', $coupon->id) }}" data-toggle="modal"
