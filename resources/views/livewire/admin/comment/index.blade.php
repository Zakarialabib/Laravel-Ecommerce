<div>
    ...
    <x-table>
        ...
        <x-table.tbody>
            @forelse($comments as $id=>$comment)
                <x-table.tr>
                    <x-table.td>
                        {{ $id }}
                    </x-table.td>
                    <x-table.td>
                        <a href={{route('front.product',$comment->product->slug)}} target="_blank">
                        {{ mb_strlen(strip_tags($comment->product->name),'utf-8') > 50 ? mb_substr(strip_tags($comment->product->name),0,50,'utf-8').'...' : strip_tags($comment->product->name) }}
                        </a>
                    </x-table.td>
                    <x-table.td>
                        {{ $comment->user->name }}
                    ...
                    <x-table.td>
                        {{ mb_strlen(strip_tags($comment->text),'utf-8') > 250 ? mb_substr(strip_tags($comment->text),0,250,'utf-8').'...' : strip_tags($comment->text) }}
                    </x-table.td>
                    <x-table.td>
                        <x-dropdown
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-32 p-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link data-href="{{ route('admin-comment-show', $comment->id) }}"
                                    class="view details-width" data-toggle="modal" data-target="#modal1">
                                    <i class="fas fa-eye"></i>
                                    {{ __('Show') }}
                                </x-dropdown-link>
                                <x-dropdown-link href="javascript:;"
                                    data-href="{{ route('admin-comment-delete', $comment->id) }}" data-toggle="modal"
