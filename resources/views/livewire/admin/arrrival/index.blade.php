<div>
    ...
    <x-table>
        ...
        <x-table.tbody>
            @forelse($arrivals as $id=>$arrival)
                <x-table.tr>
                    <x-table.td>
                        {{ $id }}
                    </x-table.td>
                    <x-table.td>
                        @php($photo = $arrival->photo ? url('assets/images/arrival/' . $arrival->photo) : url('assets/images/noimage.png'))
                        <img src="'{{ $photo }}" alt="Image">
                    </x-table.td>
                    <x-table.td>
                        @php($title = mb_strlen(strip_tags($arrival->title), 'UTF-8') > 250 ? mb_substr(strip_tags($arrival->title), 0, 250, 'UTF-8') . '...' : strip_tags($arrival->title))
                        {{ $title }}
                    </x-table.td>
                    <x-table.td>
                        @php($header = mb_strlen(strip_tags($arrival->header), 'UTF-8') > 250 ? mb_substr(strip_tags($arrival->header), 0, 250, 'UTF-8') . '...' : strip_tags($arrival->header))
                        {{ $header }}
                    </x-table.td>
                    <x-table.td>
                        <livewire:toggle-button :model="$arrival" field="status" key="{{ $arrival->id }}" />
                    </x-table.td>
                    <x-table.td>
                        <x-dropdown
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-32 p-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link href="{{ route('admin-arrival-edit', $arrival->id) }}"> <i
                                        class="fas fa-edit"></i>
                                    {{ __('Edit') }}
                                </x-dropdown-link>
                                <x-dropdown-link href="javascript:;"
                                    data-href="{{ route('admin-arrival-delete', $arrival->id) }}" data-toggle="modal"
